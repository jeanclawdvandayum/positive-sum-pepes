...
contract A4_E2ETest is Test {
    ...
    /// Full lifecycle: launch → curve trade (pot accrues) → bomb (pot
    ...
    function test_E2E_F9_flat_window_zero_fee_nothing_stranded() public {
        ...
        vm.stopPrank();
        uint256 potAtCurve = controller.potPSPBalance();
        assertGt(potAtCurve, 0, "curve buy should accrue pot");
        ...
        // bomb-time pot fully closed out: burned, backing ring-fenced
        assertEq(controller.potPSPBalance(), 0, "bomb should redeem the pot");
        uint256 factoryPotAfterBomb = factory.sidePot();
        assertGt(factoryPotAfterBomb, 0, "pot backing at factory");
        ...
        uint256 flatPot = controller.potPSPBalance();
        // F-9 FIXED (2026-08-19): zero-fee flat window — dying-round trades
        // accrue NOTHING to the pot. Pre-fix this seeded a stranded pot.
        assertEq(flatPot, 0, "F-9 fixed: flat trades accrue no pot");
        console2.log("pot PSP stranded by flat window:", flatPot);

        // custody invariant on the real contracts
        assertEq(
            pspToken.balanceOf(address(controller)),
            controller.totalLocked() + controller.potPSPBalance(),
            "H-1 custody invariant (real stack)"
        );
        ...
        // bomb-time pot WAS forwarded to round 2 (ring-fence intact for it)
        assertGt(r2.controller.totalPotMixETH(), 0, "bomb-time pot forwarded");
        assertApproxEqAbs(
            r2.controller.totalPotMixETH(), factoryPotAfterBomb, 1, "exactly the bomb-time pot"
        );
        assertEq(factory.sidePot(), 0, "factory pot emptied into round 2");

        // F-9 FIXED CORE: nothing stranded at the dead controller — the pot
        // ledger is zero and no PSP sits there beyond locker principal
        assertEq(controller.potPSPBalance(), 0, "F-9 fixed: no stranded flat pot");
        assertEq(
            pspToken.balanceOf(address(controller)),
    ...
    function test_E2E_round2_rebirth_loop_alive() public {
        ...
        assertGt(c2.potPSPBalance(), 0, "round 2 pot accrues");
    }
}
