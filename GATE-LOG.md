...
2026-09-14 — AUDIT RESOLUTIONS LANDED: L-M1 FIXED in code — RoundController constructor now rejects VEST_DURATION < 6 (TimingsIncomplete, subsumes zero-check) at the decode site; guards all deployment paths incl. direct vessel births. Regression test test_C3_RejectsSubEpochalVest: vest 1-5s hand-assembled words each revert at decode, vest=6 boundary deploys (VEST_DURATION()==6). N-1 ACCEPTED procedurally (one eternal factory, Sept-9 descriptor never touched, future art = new version alongside; frontend soft-locks folded DNA KEYS across rounds for cross-round uniqueness — chosen-ID art is round-independent keccak(id), alias ids exist so key-lock not id-lock). GATES: forge test 584 pass / 1 known WNS_FORK_RPC_URL env-fail / 3 skip (C3 suite 9/9 incl. new regression + EIP-170 sizes unchanged: RoundController 12.8kB/24.6kB). Resolutions appended to audits/2026-09-13-deployment-spec/REPORT.md.

2026-09-14 — GRAVEYARD ONE-CLICK EXIT + BADGE RELIABILITY: (1) src/PSPGraveZap.sol NEW — exit(hook, staker, pepeIds, pspIn, minMixOut, deadline) one-tx exit: hook.claimPotFor(user) pot→user direct, staker.claimAllTo fees→user (zero-guarded), staker.withdrawFor per-pepe unlock PSP→owner, hook.redeemBacking fee-free pro-rata redemption with proceeds forwarded. Holds no funds; unstaked ids fail loud. (2) CurveHook: +claimPotFor(address) (pays the seat owner, anyone may call — third parties can only CREDIT the owner); claimPot/claimDeployerCredit deduped via _payMix → CurveHook 23,918B (WAS 24,019B pre-change; 500B-headroom gate green). (3) PSPStaker: +withdrawFor(pepeId) operator path — principal AND fees always to the OWNER (operators already hold strictly more power via NFT transfer; no new theft surface). (4) B6_GraveZap.t.sol 7/7 on the real-pool rig (real launch/buys/locks/detonation; minOut atomicity; stranger rejection; owner-only payouts; zero-fee skip; deadline). NOTE: harness router buys attribute seats to its internal swapper (trader = hookData identity) — B6 buys via zapIn.buyWithMix. (5) Frontend: graveyard badges now archive-state-independent (Transfer-log-derived settlement owner, 50k-block windows, per-hook winners cache) — BOTH public Base Sepolia RPCs prune historical state within days (all 3 dead rounds already unreadable via ownerOf-at-block); DeadRoundCard gained "exit round — one transaction" (VITE_GRAVE_ZAP-gated) with the exact line "claim X mixETH · unlock Y PSP · convert P PSP → Z mixETH", approvals bundled via writeWithApprovals (atomic wallets = one signature). vite/tsc build clean. LIVE-TESTNET CAVEAT: claimPotFor/withdrawFor exist only in NEW bytecode — deployed Sept-7-factory rounds keep the granular flow; the zap activates with the eternal factory deploy + VITE_GRAVE_ZAP.

2026-09-14 — SINGLE-SINE SPEC v3 (`single-sine-spec`): one continuous tilted-sine curve through IBCO and active trading; softened cube-root growth (1,000× launch spot by the tenth wave); pot-priced tickets at pot/10,000 with the active minimum exactly one spot; sqrt wavelength scaling (lam = 955·√(boot/450)); reserve domain target+64 waves with explicit SineV3Domain capacity errors; ONE shared read-only SineV3Table (12,128 B) pinned in the factory from genesis. Ticket-intent guard: 64-byte hookData {trader, maxTicketPrice} — guarded zap/registry routes revert TicketPriceMoved when the pot reprices between quote and settlement.

GATES (homebrew node 25 in PATH — /usr/local node 20 rejects --experimental-strip-types): forge **600 passed / 0 failed**; slither 0 High / 0 Medium; no-governance clean; sizes **42 artifacts** — CurveHook runtime **20,228 B** (24,019 pre-v3), SineV3Math 12,128 B; independent Decimal and generated-knot sine oracles match; **ABI 172**; **185 frontend + 11 name node tests**; tsc/vite build and build:alt green.

LOCAL ANVIL LIFECYCLE (fresh chain, disposable keys): **17/17 deployment receipts** — SineV3Math deploys in its own broadcast before the factory (fixes the nested-broadcast revert); **44 checked action receipts** (38 success, 6 expected reverts incl. the below-minimum buy and the domain-piercing whale overspend: below the tenth-wave target buys skim 10% to fees, so the overspend is gross·10/9 over the curve headroom); two rounds — 2,000-mix genesis (boot 1,800, lam 1,910, target 20,900, q0 ≈ 57,318 PSP, launch spot 0.02) and 5,000-mix rebirth (boot 4,500, lam 955·√10 ≈ 3,020, q0 ≈ 202,528, spot 0.05); sell fees reprice tickets without adding seats or time; **35 local-fork tests** green incl. the solvency invariant (64 runs / 2,048 calls, 0 reverts).

Settlement bug fixed en route: the sell inverter's proportional seed was subtracted from R instead of used as the candidate — sells of ~0.2% of supply reverted ZeroOutput (caught by the custody fuzz; now matches manual bisection to the wei). No public deployment, nothing published; existing rounds unchanged. Spec: docs/FINAL-SPEC.md; notes: docs/audit/2026-09-14-single-sine-v3.md.



2026-09-15 — SINGLE-SINE-SPEC REVIEW: RELEASE BLOCKED. Reviewed a80984c8 against the original handoff and e8d24e4f baseline, including the intervening graveyard router. Fixed locally: approved-zap fee redirection, incomplete sell inverse, buy-quote range checks, authenticated reinvestment hook, v3 deployment/manifest/source-verification tooling, inactive chart cache, exact dynamic minima, and stale explainer math. Tightened oracle tolerances without changing reference data. Three required adjacent-wei monotonicity regressions remain failing; fixed -16/+64 table bounds also remain an unapproved spec deviation.

GATES: bash scripts/check-audit.sh exits 1 — 607 Solidity tests pass, 3 fail, 0 skip, 81 suites (610 total). All failures are SineV3ReviewRegressionTest price/supply monotonicity pins. Remaining deterministic checks ran separately: art/renderer fixtures pass; no-governance clean; 42 production size checks pass; legacy oracle and v3 generated files match; ABI 172; frontend 207/207; name tests 11/11; TypeScript and normal/alternative Vite builds pass. Vite retains its large-chunk advisory. Runtimes: CurveHook 20,261 B; SineV3Math 12,162 B; PSPFactory 19,795 B; PSPReinvestor 5,297 B. No gate was weakened or failing assertion excluded.

LOCAL LIFECYCLE: fresh disposable Anvil on chain31337 — 18/18 deployment receipts, 46 checked actions (39 success, 7 expected revert), 27 runtime/data checks, two rounds. Maximum action gas 9,797,172. Separate funded real-V4 inverse regression passes; repaired wave63 sale uses 6,372,725 gas. Browser desktop/mobile checks pass for updated paper and explainers. Slither unavailable on PATH; no new static-analysis or live-fork result claimed. No public deployment, publication, live-address change, or historical broadcast replacement. Report: docs/audit/2026-09-15-single-sine-review.md. Durable receipt evidence: docs/audit/2026-09-15-single-sine-evidence/.

2026-09-15 — REVIEW FIXES + MONOTONE NUMERICAL CORE (external review + redesign): an external agent's review of `a80984c8` found 2 High / 3 Medium / 3 Low. All eight resolved. (1) **AUD-V3-1 non-monotone supply/price** — the GL8 moving-node partial and rounded closed form could DECREASE at adjacent reserve wei (measured: −12,733,333 PSP wei at the 500-mix calibration; price −3 wei at wave 16.5). Replaced with a **Bernstein monotone core**: `SineV3Primitive` (graded cells from phase −2320 to +16384 in 4-units, degree-17 density controls, floor de Casteljau — provably nondecreasing at adjacent wei; knots in 4 codehash-pinned immutable data shards, 21,993/22,001/22,001/18,705 B) and `SineV3Price` (monotone Bernstein quarters for s(x), exact floored cbrt, positive-coefficient Horner Taylor exp with ln2 range reduction — every rounded stage preserves order; **no wave cutoff**, phase bound = representability). `SineV3Math` runtime 14,182 B. All six review regression pins (SineV3ReviewRegression) PASS and stay in the suite. (2) **AUD-V3-2 sell underpayment** — Newton without forced bracket halving returned an unresolved bound (wave-63 sale paid 0.000006% of true output): ≤16 Newton steps then forced bisection, ≤1-wei bracket + minimality check on exit, fail-closed `SineV3InverseDidNotConverge`. (3) **AUD-V3-3 table-defined limits** — `BOOT_NET_MAX` REMOVED; domain now DERIVED (docs/audit/2026-09-15-single-sine-domain.md): negative edge −580.5 waves from the positive-opening-price inequality (boot < ~681.9M mixETH at p=1e18), positive edge +4096 waves from a certified sub-wei reciprocal tail; genesis admission requires Q ≥ share denominator (every positive deposit claims ≥ 1 wei, proven + tested incl. one-wei launches at both price endpoints). (4) **AUD-V3-5 GraveZap fee redirect** via arbitrary NFT ids — `ownerOf(id) == msg.sender` for every requested position. (5) AUD-V3-6 v2 remnants in deploy tooling (bounded PSP_SINE_PL, versioned manifest incl. shard identity); (6) AUD-V3-4 stale charts/minima (retries, exact live minimum, v2 math purged from all pages); (7) AUD-V3-7 quote bounds = execution bounds; (8) AUD-V3-8 reinvestor authenticates the round's hook before reads. GATES: `check-audit.sh` PASS — forge **715 gated suites** (89 suites; full run 725/93 incl. integration, 1 known WNS env-fail), three new validator legs (`sine_v3_price`, `sine_v3_small_reserve` 90-digit Simpson oracle at phases to −579, `sine_v3_primitive_validate`), sizes (CurveHook **20,361 B**), ABI, 208 frontend + 11 names, builds. LOCAL ANVIL LIFECYCLE: **21/21 deployment receipts** (14 creates incl. 4 shards, byte-identical to `sine_v3_data.json`), **46 action receipts** (39 ok / 7 expected reverts), every tx under the 2^24 gas cap, 35 local-fork tests incl. solvency invariant. Evidence: docs/audit/2026-09-15-single-sine-evidence/. Existing deployments unchanged; not deployed publicly. Review doc: docs/audit/2026-09-15-single-sine-review.md; fixes: docs/audit/2026-09-15-single-sine-fixes.md. The release block above is lifted by this entry: every finding is fixed, pinned, and gated.


## 2026-09-16 — evm-cortex re-audit artifacts (report-only)

- Audit commit `32659c22` on top of the reviewer's `471b0443` (pre-launch
  minimum fix). No src/ or script/ changes; adds
  `test/SineV3AuditPoC.t.sol` (+6 tests: buy/sell quote-vs-execution parity,
  TicketPriceMoved guard end-to-end, pre-fee ticket sampling, binding ladder
  minimum, sell-side dust guard) and
  `docs/audit/2026-09-16-single-sine-evm-cortex.md`. Verdict: no new
  exploitable findings.
- `bash scripts/check-audit.sh`: **PASS** — **721 Solidity tests / 90 suites**
  (715 + 6 over the Sept-15 baseline), **211 frontend tests**, names tests,
...
2026-09-22 ~14:45 +08 — FORMAL ESCALATION CAMPAIGN (test-side only; no production changes): unresolved-scope assault on the EVM tier. Harness upgrades: ticket specs re-engineered to addition-form (via-IR lowers sub-under-<= into a SYMBOLIC-DIVISOR division — the single monster query per property; receipts preserved), wrap-guard + exhaustive concrete check of the top 10,003 pots on the etched runtime (test_ticket_interval_top_region_exhaustive), cell 712 added (symbolic check + fuzz + edge anchors + strict-knot list — with 711 it covers both graded cells the canonical buyOut launch row reads) and registered in the formal EXTENDED registry. Escalated attempts: 20x wall (1800s), 10-15x solver (300s, probes 1800s), z3 serial + 8-thread, bitwuzla 0.8.1, cvc5 1.2.1, yices 2.6.4 (halmos-native), isolated minimal-implementation diagnostic. RESULTS: ticket interval/adjacent + phase q1/q2 WALL_TIMEOUT at 30 min on the v2 harness; buyout pair re-blocked on halmos 0.3.3 NotConcreteError (symbolic EXTCODECOPY offset); NO counterexample from any engine at any budget; core 15/15 sanity green. Root cause + receipts + composition closures: docs/audit/2026-09-22-formal-escalation.md (+ evidence dir, incl. saved SMT2 queries from every timed-out run and aborted-first-run summary.json). GATES on the working tree: forge 790/790 PASS (97 suites, 0 skipped — +2 from the new tests: top-region exhaustive + fuzz_712); frontend 211 PASS; names 11; size gate 49 production artifacts (delta vs 50 on 9/21 is out/-glob accounting over test compilation units — every src/ file present and size-legal); ABI/art/no-governance/TS/Vite green (Vite chunk advisory unchanged). Working tree also carries the campaign docs; commits held for scoopy's publish signal.

2026-09-22 ~21:50 +08 — FRESH BASE SEPOLIA ROUND PREP (scoopy go; commits 7f3a0e2c receipts + a1004255 e2e rebirth drive, local). GATES: fresh check-audit.sh run exit 0 — forge **790/790 in 97 suites**, formal identity pass, art/no-governance/sizes/sine-oracles green, frontend node+tsc+vite green, build:alt green. SLITHER (first full run on the sine branch — the 0H/0M baseline was pre-sine deployment-spec; every entry since said "unavailable on PATH"): 287 findings = accepted triage set (all 3 High byte-identical to docs/audit triage CSVs) + 12 new Medium+ fingerprints, each adjudicated benign (phaseAt divide-before-multiply = floored decomposition, oracle bit-exact + seams proved; 2x uninitialized-local = zero-init language semantics; names-subsystem unused-return cluster + PSPGraveZap local = peripheral, prior-wave audited) — **no new real findings**. ANVIL E2E: **19/19 receipts** — genesis → predeposit ×2 → launch → buy → claims → detonate (82,719 gas halt+flatten+mark) → **AUD-3 permissionless rebirth** (reserveSpawn 2,661,982 gas + birthStep 9,815,487 / 8,923,103 / 531,444, every leg under Base Sepolia's ~16.78M per-tx cap) → round-2 == reservation → R2 predeposit/launch/buy → round-2 ALIVE. First drive of the true production rebirth path — the old script's in-tx-birth assumption never happens under a production gas cap (fixed in a1004255; reserveSpawn retries across blocks for fresh entropy since the bounded mine cheap-fails). Env note: this anvil build's evm_setTime disables automine — re-arm post-probe or forge wedges on receipts. RUNNER FORK REHEARSAL (scripts/deployment/base-sepolia.mjs @ a1004255, soak profile 3600/3600/3600/0, public RPC): **passed** — isolated cold-cache gate (97 suites) → fresh factory + 3-step genesis → staged addresses from receipts → reinvestor 2nd pass → manifest + frontend.env export → BaseSepoliaReleaseTest 2/2 on the deployed bytecode; 22 txs / 77,751,056 gas; receipts under out/releases/base-sepolia-rehearsal-2026-09-22T08-43-30-168Z/. LIVE DEPLOY staged: keystore psp-testnet (deployer 0x1d52…d88a, 0.2577 ETH) — awaiting scoopy's keystore import; live env staged at .env-live-deploy-staged. Push to origin still rides the publish signal.
