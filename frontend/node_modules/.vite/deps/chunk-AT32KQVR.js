...
function getSignPayload(value) {
  ...
  return keccak2562(concat2(fromString(signatureDomain), from2(payload), account));
}
...
function deserialize_(value, multisigDepth) {
  ...
  if (typeId === serializedMultisigType) {
    ...
    for (const signature of signatures)
      if (size(signature) > maxOwnerSignatureBytes)
        throw new InvalidSerializedError({
          reason: `multisig owner signature exceeds ${maxOwnerSignatureBytes} bytes`,
          serialized
        });
...
var InvalidSignatureTypeError = class extends Error {
  constructor(type2) {
    super(`Signature type \`${type2}\` is invalid for key authorizations; expected \`secp256k1\`, \`p256\`, or \`webAuthn\`.`);
    Object.defineProperty(this, "name", {
      enumerable: true,
      configurable: true,
      writable: true,
...
async function serializeTempo(transaction, sig) {
  ...
  const signature = (() => {
    ...
    return SignatureEnvelope_exports.from({
      genesisConfig: transaction.multisig,
      signatures: sorted,
      ...nonce2 || transaction.nonceKey ? {} : { init: true }
    });
  })();
  ...
  prepareTransactionRequest: [
    async (r, { client, phase }) => {
      ...
      if (multisig) {
        ...
        request.multisigSignatureCount ?? (request.multisigSignatureCount = inferMultisigSignatureCount(config));
...
//# sourceMappingURL=chunk-AT32KQVR.js.map
