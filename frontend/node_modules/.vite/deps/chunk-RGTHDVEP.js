...
async function scheduleMulticall(client, parameters) {
  ...
  const { schedule } = createBatchScheduler({
    ...
    shouldSplitBatch(calls) {
      if (batchSize === 0)
        return false;
      const size4 = calls.reduce((size5, { callData }) => size5 + (callData.length - 2) / 2, 0);
      return size4 > batchSize;
    },
...
var transferEventSelector = getSelector2(from2("event Transfer(address indexed from, address indexed to, uint256 value)"));
var balanceOfFunction = from4("function balanceOf(address) returns (uint256)");
var decimalsFunction = from4("function decimals() returns (uint256)");
var tokenUriFunction = from4("function tokenURI(uint256) returns (string)");
var symbolFunction = from4("function symbol() returns (string)");
var staticCallFunction = from4("function query(address target, bytes data)");
async function simulateCalls(client, parameters) {
  const { blockNumber, blockTag, calls, stateOverrides, traceAssetChanges, traceTransfers, validation } = parameters;
  const account = parameters.account ? parseAccount(parameters.account) : void 0;
  if (traceAssetChanges && !account)
    throw new BaseError("`account` is required when `traceAssetChanges` is true");
  const getBalanceData = account ? encode2(from3("constructor(bytes, bytes)"), {
    bytecode: deploylessCallViaBytecodeBytecode,
    args: [
      getBalanceCode,
      encodeData(from4("function getBalance(address)"), [account.address])
    ]
  }) : void 0;
...
function getCall2(client, parameters) {
  const { amount, from: from5, to, token } = parameters;
  const { address, decimals } = resolveToken(client, { token });
...
//# sourceMappingURL=chunk-RGTHDVEP.js.map
