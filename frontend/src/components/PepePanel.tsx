  ...
  if (tokenId === undefined) {
    return (
      <div className="card p-5">
        <div className="h-48 animate-pulse rounded-2xl bg-sky-50" />
      </div>
    )
  }
  ...
  return (
    <div className="card flex flex-col items-center p-5 text-center">
      <div
        ...
        style={svg ? undefined : { filter: 'grayscale(1) opacity(0.35)' }}
      >
        {svg ? (
      ...
      <p className="mt-2 text-xs font-bold text-emerald-600">
        {artOnline ? 'rendered on-chain · yours forever' : 'art renderer not wired yet'}
      </p>
    </div>
  )
}
