...
export default function AltLanding() {
  ...
  }} onInput={e=>{const target=e.target as HTMLInputElement;if(target.id==='pot-slider'){ref.current?.querySelectorAll<HTMLElement>('[data-payout-bar]').forEach((el,i)=>{el.textContent=`${(Number(target.value)*[25,18,14,10,8,7,6,5,4,3][i]/100).toFixed(2)} mix`});const out=ref.current?.querySelector('#pot-output');if(out)out.textContent=`${target.value} mixETH`}}}><div dangerouslySetInnerHTML={content}/>{slot&&createPortal(<Instrument/>,slot)}</div>
}
